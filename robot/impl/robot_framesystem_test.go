...
// TestModularGripperGeometriesInFrameSystem asserts that a modular gripper which implements
// `Geometries` has that geometry incorporated into the robot's frame system. A gripper is a
// ModelFramer, so the gripper client's `Kinematics` builds a model from the component's
// `Geometries`, and that geometry surfaces in the frame system where motion planning would use it.
func TestModularGripperGeometriesInFrameSystem(t *testing.T) {
	ctx := context.Background()
	logger := logging.NewTestLogger(t)

	testGripperModel := resource.NewModel("rdk", "test", "gripper")
	testPath := rtestutils.BuildTempModule(t, "module/testmodule")

	// Parent the gripper to the world frame with no offset so the geometry's world-frame pose
	// equals its pose relative to the gripper's own frame, letting us compare the two directly.
	cfg := &config.Config{
		Components: []resource.Config{
			{
				Name:  "modgripper",
				API:   gripper.API,
				Model: testGripperModel,
				Frame: &referenceframe.LinkConfig{
					Parent: referenceframe.World,
				},
			},
	...
	// The gripper's geometry rides along in its kinematic model (part.ModelFrame), which the
	// gripper client's `Kinematics` builds from the modular component's `Geometries`.
	fsCfg, err := r.FrameSystemConfig(ctx)
	test.That(t, err, test.ShouldBeNil)
	fs, err := referenceframe.NewFrameSystem("test", fsCfg.Parts, nil)
	test.That(t, err, test.ShouldBeNil)

	// FrameSystemGeometries returns every frame's geometries transformed into the world frame. The
	// only geometry in this frame system is the gripper's.
	geomsByFrame, err := referenceframe.FrameSystemGeometries(fs, referenceframe.FrameSystemInputs{})
	test.That(t, err, test.ShouldBeNil)

	var fsGeoms []spatialmath.Geometry
	for _, gif := range geomsByFrame {
		fsGeoms = append(fsGeoms, gif.Geometries()...)
	}
	test.That(t, fsGeoms, test.ShouldHaveLength, 1)

	// The geometry in the frame system should match what the gripper reports for itself.
	grip, err := gripper.FromProvider(r, "modgripper")
	test.That(t, err, test.ShouldBeNil)
	gripGeoms, err := grip.Geometries(ctx, nil)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, gripGeoms, test.ShouldHaveLength, 1)

	test.That(t, fsGeoms[0].ToPoints(1), test.ShouldResemble, gripGeoms[0].ToPoints(1))
}
