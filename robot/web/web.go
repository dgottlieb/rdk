...
func (svc *webService) StartModule(ctx context.Context) error {
	...
	unaryInterceptors = append(unaryInterceptors, logging.ModNameUnaryServerInterceptor)
	f := func(ctx context.Context, req any, info *googlegrpc.UnaryServerInfo, handler googlegrpc.UnaryHandler) (resp any, err error) {
		...
		case *streampb.AddStreamRequest:
			...
			svc.logger.Infof("rdks mod stream server. AddStream: %v TrackName: %v", name, r.SDPTrackName())
			if err != nil {
				break
			}
		...
		case *streampb.RemoveStreamRequest:
			...
			svc.logger.Infof("rdks mod stream server. RemoveStream: %v TrackName: %v", name, r.SDPTrackName())
			reqA.Name = r.SDPTrackName()
			req = reqA
		default:
			return handler(ctx, req)
		...
		svc.resourceNameToPeerConnectionMu.Lock()
		modName := logging.GetModuleName(ctx)
		svc.logger.Info("ModServer interceptor. ModName: ", modName)
		pc, ok := svc.resourceNameToPeerConnectionMap[modName]
